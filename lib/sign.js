...
exports.prepareForSave = function(sign) {
  return Q.fcall(function() {
    if (sign.programs === undefined) sign.programs = [];
    sign.programs = _.uniq(sign.programs);
...
/**
 * Gets the programs for a sign.
 * @param  {Object}   sign
 * @return {Q(Array)}
 */
exports.programs = function(sign) {
  return db()
  .then(function(r) {
    var filter = {};
    // filter[program.idFieldName()] = sign.programs;
    return db.qcollect(r.table(program.tableName()).filter(filter).run());
  });
};